...
exports.myOrders = asyncHandler(async (req, res) => res.json(new ApiResponse("Orders fetched", { orders: await Order.find({ customer: req.customer._id }).populate("vendor", "name email role").sort("-createdAt") })));
exports.myOrder = asyncHandler(async (req, res) => res.json(new ApiResponse("Order fetched", { order: await Order.findOne({ _id: req.params.id, customer: req.customer._id }).populate("vendor", "name email role") })));
